...
public class RequestServiceImpl implements RequestService {
    ...
    public List<ItemRequestDtoOutput> getAll(long userId) {
        User user = userRepository.findById(userId).
                orElseThrow(() -> new NullObjectException("Ошибка проверки " +
        ...
        if (items.size() > 0) {
            for (ItemRequest itemRequest : requests) {
                List<ItemDtoRequestAnswer> itemsDto = items.stream().filter(item ->
            ...
            itemRequestDtoOutputs = requests.stream().map(itemRequest ->
    ...
    public List<ItemRequestDtoOutput> getAllOtherSort(long userId, Sort sort) {
        ...
        if (items.size() > 0) {
            ...
            itemRequestDtoOutputs = request.stream().map(itemRequest ->
                            requestMapper.toItemRequestDtoOutputNullRequest(itemRequest)).collect(Collectors.toList());
        }
        return itemRequestDtoOutputs;
    }
